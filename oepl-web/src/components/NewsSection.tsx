...
export default function NewsSection() {
  ...
  const { latestId, items } = useMemo(() => {
    ...
    return { latestId: latestNewsId(content.news), items: sorted.slice(0, 4) };
  }, [content.news]);
  return (
        ...
        <div className="flex flex-row gap-5 overflow-x-auto pb-2 -mx-6 px-6 snap-x snap-mandatory md:grid md:grid-cols-4 md:overflow-visible md:mx-0 md:px-0">
          {items.map((item) => {
            ...
            <Link
              key={item.id}
              href={`/news/${item.id}`}
              className="card-hover flex-shrink-0 w-[280px] sm:w-[260px] md:w-auto rounded-2xl overflow-hidden border border-gray-200 bg-white flex flex-col group cursor-pointer shadow-sm snap-start hover:border-[#E88800]/40"
            >
              <div className="relative h-44 overflow-hidden bg-gray-50">
                {display.coverPhoto ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={display.coverPhoto} alt="" className="w-full h-full object-cover" />
                ) : (
              ...
              <div className="px-4 py-3 flex flex-col gap-1.5">
                ...
                <h3 className="text-[var(--color-neutral-900)] font-semibold text-xs leading-snug line-clamp-1 flex items-center gap-1.5">
